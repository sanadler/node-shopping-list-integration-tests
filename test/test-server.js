...
describe("Recipes", function() {
  ...
  it("should add an recipe on POST", function() {
    const newRecipe = { name: "burrito", ingredients: [
      "beans",
      "beef",
      "peppers"
    ] };
  ...
  it("should update recipe on PUT", function() {
    const updateData = {name: "foo", ingredients: [
      "bizz",
      "buzz",
      "fizzbuzz"]
    };
    ...
    return (
      chai
        ...
        .then(function(res) {
          expect(res).to.have.status(204);
          //expect(res).to.be.json; -- based on code it is not turned into json
          expect(res.body).to.be.a("object");
         // expect(res.body).to.deep.equal(updateData); -- based on code, recipe put doesnt return the object
        })
    );
  });
